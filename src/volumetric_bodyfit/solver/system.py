...
from volumetric_bodyfit.fieldnets.volume_query import BasisPointEncoder, GlobalPointEncoder, TemplateBasisBank, VoxelOffsetField, WideVoxelOffsetField, ScaledVoxelOffsetField
from volumetric_bodyfit.config.runtime import neutral_body_model_path
...
######## UTILITY FUNCTIONS #############
def build_colored_geometry_figure(
    verts,
    trivs,
    colors=None,
...
def make_match_visualization_arrays(src, tar, m):
    c_src = coordinate_palette(src)
...
def coordinate_palette(src, freq=1):
    funz_ = torch.dstack(
        [
...
def render_colored_match_cloud(res):
    fig = build_colored_geometry_figure([res[:, 0:3]], [None], colors=[res[:, 3:]])
    return fig
...
def soft_correspondence_loss(basis_A, basis_B, pc_B):
...
def basis_alignment_loss(basis_A, basis_B, pc_B):
...
def descriptor_transport_loss(phi_A,phi_B,G_A,G_B):
        # Computing optimal transformation
...
class FieldRegistrationModule(pl.LightningModule):
    logger: NNLogger
    ...
    def load_runtime_options(self, kwargs):
        ...
        if self.if_templ:
            bm = BodyModel(bm_fname=neutral_body_model_path, num_betas=num_betas)
    ...
    def __init__(self, metadata: Optional[DatasetProfile] = None, *args, **kwargs) -> None:
        super().__init__()
        ...
        if metadata is None:
            ...
            self.occ_res = metadata.shape_spec()['occ_res']
            self.gt_points = metadata.shape_spec()['gt_points']
            self.idxs = metadata.shape_spec()['idxs']
            if len(metadata.shape_spec()['labels'])>0:
                self.labels = metadata.shape_spec()['labels'][self.idxs]
            else:
                self.labels = []
        
        # FETCH THE KWARGS FROM THE YAML
        self.load_runtime_options(kwargs)
        ...
        if self.paradigm in ["LVD","LoVD"]:  # LVD: A model, with a learnable context, and a query method for novel PCs
            if self.powerup==1:
                self.model = WideVoxelOffsetField(self.size_layers, self.gt_points*3, res=self.occ_res, input_dim=self.input_dim , b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]), selfsup=self.selfsup, segm=self.segm, labels=self.labels, unsup=self.unsup)            
            elif self.powerup==2:
                self.model = ScaledVoxelOffsetField(self.size_layers, self.gt_points*3, res=self.occ_res, input_dim=self.input_dim , b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]), selfsup=self.selfsup, power_factor = self.power_factor, segm=self.segm)   
            else:
                self.model = VoxelOffsetField(self.size_layers, self.gt_points*3, res=self.occ_res, input_dim=self.input_dim , b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]), selfsup=self.selfsup, segm=self.segm)
        ...
        if self.paradigm == "Uni":  # Universal embedding: A network for incoming PC and one network for the Template
            self.model = WideVoxelOffsetField(self.size_layers, self.gt_points*3, res=self.occ_res, input_dim=self.input_dim , b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]), selfsup=self.selfsup, segm=self.segm, labels=self.labels, paradigm = 'Uni')

        # LIE Baseline
        if self.paradigm == "LIE":  # LIE: We learn basis and Descriptors
            self.models = {}
            self.models['basis'] = WideVoxelOffsetField(self.size_layers, self.n_basis, res=self.occ_res, input_dim=self.input_dim , b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]), selfsup=self.selfsup, segm=self.segm, labels=self.labels).cuda()
            self.models['desc']  = WideVoxelOffsetField(self.size_layers, self.n_desc, res=self.occ_res, input_dim=self.input_dim , b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]), selfsup=self.selfsup, segm=self.segm, labels=self.labels).cuda()
        ...
        if self.behave:
            import pickle
            from volumetric_bodyfit.config.runtime import behave_laplacian_path

            file = open(behave_laplacian_path, 'rb')
   ...
   # USEFUL FUNCTION FOR LIE BASELINE
    def switch_embedding_branch(self,mode):
    ...
    def shared_training_pass(self, batch, test=False) -> Mapping[str, Any]:
        self.count_steps += 1
        ...
        if self.paradigm == "LVD":
            ...
            if test:
                ...
                with torch.no_grad():
                    ...
                    m = self.nearest_vertex_assignment(self._target_smpl[0], input_points[0])
                    src, tar, both = make_match_visualization_arrays(self._target_smpl[0:1], input_points[0:1], np.expand_dims(m,0))
        ...
        if self.paradigm == "Uni":
            ...
            # Compute Loss
            loss = soft_correspondence_loss(pred_src, pred_tar, tar)
            log["loss_l2"] = loss
            log["loss"] = loss
            
            if test:
                with torch.no_grad():
                    m = self.nearest_vertex_assignment(pred_src, pred_tar)
                    ...
                    src, tar, both = make_match_visualization_arrays(src, tar, m)
        ...
        if self.paradigm == "LIE":
            ...
            if self.flag_mode == 'basis':
                ...
                tar =  self._target_smpl[1:]
                src =  self._target_smpl[:-1]
                loss = basis_alignment_loss(pred_src, pred_tar, tar)
                log["loss"] = loss
                log["loss_l2"] = loss
            ...
            if self.flag_mode == 'desc':
                ...
                loss = descriptor_transport_loss(basis_A, basis_B, desc_A, desc_B) 
                log["loss"] = loss
                log["loss_l2"] = loss
            ...
            # if test:
            #     with torch.no_grad():
            #         m = self.descriptor_based_assignment(basis_A,basis_B,desc_A,desc_B)
            #         ct_np = tar.detach().cpu().numpy()
            #         rec_errs = [np.sqrt(np.sum((ct_np[i, m[1][i]] - ct_np[i]) ** 2)) for i in range(ct_np.shape[0])]
            #         m_error = np.mean(rec_errs)
            #         src, tar, both = make_match_visualization_arrays(src, tar, m)
    ...
    def prepare_batch_tensors(self, input):
    ...
    def nearest_vertex_assignment(self, src, tar):
    ...
    def descriptor_based_assignment(b1,b2,d1,d2):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> Mapping[str, Any]:
        ...
        # Step
        step_out = self.shared_training_pass(batch)
    ...
    def test_step(self, batch: Any, batch_idx: int) -> Mapping[str, Any]:
        ...
        # Step
        step_out = self.shared_training_pass(batch, test=True)
        ...
        fig = render_colored_match_cloud(step_out["both"][0])
