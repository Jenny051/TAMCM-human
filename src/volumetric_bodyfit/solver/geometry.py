...
sys.path.append("./data/preprocess_voxels/libmesh_to_occupancy_grid")
sys.path.append("./data/preprocess_voxels/")
sys.path.append("./data/")

from preprocess_voxels import voxels
#from chamferdist import ChamferMetric

class SmplPoseOptimizer(torch.nn.Module):
    def __init__(self):
        super(SmplPoseOptimizer, self).__init__()
...
class ChamferMetric(torch.nn.Module):
    def __init__(self):
        super(ChamferMetric, self).__init__()
...
################# Voxalize Function
def mesh_to_occupancy_grid(mesh, res):
    # Center the shape
...
#####
def make_volume_coordinates(resX, resY, resZ, b_min=np.array([0, 0, 0]), b_max=np.array([1, 1, 1]), transform=None):
    '''
...
def mesh_to_distance_volume(scan, res):
    ...
    with torch.no_grad():
        ...
        coords, mat = make_volume_coordinates(resolution, resolution, resolution, b_min, b_max)
...
def fit_body_model_to_field(SMPL_model, in_points, gt_idxs, prior, iterations = 1000):
    # Hyperparameters
    ...
    parameters_smpl = SmplPoseOptimizer().cuda()
...
def adapt_field_on_scan(module, input_points, voxel_src, gt_points,steps=10, lr_opt=0.00001):
    optimizer = torch.optim.Adam(module.parameters(), lr=lr_opt)

    for i in tqdm.tqdm(np.arange(0,steps),desc="BODYFIT"):
...
def refine_body_with_chamfer(SMPL_model, pred_mesh, vertices_scan, prior,init, bidir=0):
    chamferDist = ChamferMetric()
    parameters_smpl = SmplPoseOptimizer().cuda()
...
def transfer_matches_from_field(s_src, s_tar, reg_src, reg_tar):
...
def encode_scan_volume(scan, res, style='occ', grad=0,device='cuda', margin=0.8,center=True,scale=True):
    if style=='occ':
        voxel_src, mesh, total_size, centers = mesh_to_occupancy_grid(scan, res) #self.mesh_to_occupancy_grid_scan(scan)
    else:
        voxel_src, mesh, total_size, centers = mesh_to_distance_volume(scan, res)
...
def integrate_field_vertices(module, gt_points, voxel_src, iters=20, init=None):
    with torch.no_grad():
        if init is None:
...
def threshold_recall_curve(errors, thresholds):
...
import itertools
def adapt_full_network_on_scan(module, voxel_src, input_points, gt_points):
    module.train()  
...
class SurfaceOffsetOptimizer(torch.nn.Module):
    def __init__(self,n_v=6890):
        super(SurfaceOffsetOptimizer, self).__init__()
...
def refine_surface_offsets(out_cham_s, SMPL_model, target, lambda_d1=0.01, lambda_lapl = 10000, lambda_reg = 5.0, iterations=1000,lr = 1e-4, subdiv=0):
    with torch.no_grad():
        faces = SMPL_model.faces
        L, M = robust_laplacian.mesh_laplacian(np.asarray(out_cham_s), np.asarray(faces))
        L = L.todense().astype(np.float32)
    smpld_vertices, params = optimize_surface_offsets(L, out_cham_s, faces , target, lambda_d1, lambda_lapl, lambda_reg,iterations,lr)
    p = {}
    p[0] = params    
    ...
    if subdiv:
        for j in range(subdiv):
            ...
            L = L.todense().astype(np.float32)           
            smpld_vertices, _ = optimize_surface_offsets(L, out_cham_s, faces , target, lambda_d1, lambda_lapl, lambda_reg,iterations,lr)
            #p[j+1] = params_2
...
def optimize_surface_offsets(L, out_cham_s, faces, target, lambda_d1=0.01, lambda_lapl = 10000, lambda_reg = 5.0,iterations=1000,lr = 1e-4):
    ...
    with torch.no_grad():
        ...
        parameters_offsets = SurfaceOffsetOptimizer(out_cham_s.shape[0]).cuda()
...
def solve_procrustes_alignment(X, Y, scaling=True, reflection='best'):

    """

    A port of MATLAB's `solve_procrustes_alignment` function to Numpy.
    ...
    as the goodness of fit criterion.
        ...
        d, Z, [tform] = solve_procrustes_alignment(X, Y)
