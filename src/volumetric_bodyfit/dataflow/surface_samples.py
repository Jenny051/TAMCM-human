...
from volumetric_bodyfit.config.runtime import neutral_body_model_path, processed_training_data_dir
...
class SmplFieldDataset(Dataset):
    def __init__(
        self,
        mode,
        num_betas=10,
        ...
        **kwargs
        ...
        # Set DATAPATH
        data_root = data_path or processed_training_data_dir
        self.path = os.path.join(data_root, kwargs["version"], "stage_III")
    ...
    def shape_spec(self):
        return {
            "occ_res": self.occ_res,
            "gt_points": len(self.idxs),
            "labels": self.labels,
            "idxs": self.idxs,
        }
