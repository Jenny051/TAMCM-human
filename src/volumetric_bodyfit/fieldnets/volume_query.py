...
class TemplateBasisBank(nn.Module):
    def __init__(self, n_points=1000, n_basis=20, batch_norm=False):
        super(TemplateBasisBank, self).__init__()
...
class GlobalPointEncoder(nn.Module):
    def __init__(
        self,
        n_points=500,
        n_in=3,
        ...
        super(GlobalPointEncoder, self).__init__()
...
class FieldNetworkBase(nn.Module):
    def __init__(self):
        super(FieldNetworkBase, self).__init__()
...
class VoxelOffsetField(FieldNetworkBase):
    def __init__(self, hidden_dim=512, output_dim=6890, res=128, input_dim=1, 
        ...
        super(VoxelOffsetField, self).__init__()
...
class WideVoxelOffsetField(FieldNetworkBase):
    def __init__(self, hidden_dim=512, output_dim=6890, res=128, input_dim=1, 
                 b_min = np.array([-1.2, -1.4, -1.2]), b_max = np.array([1.2, 1.3, 1.2]), 
                 paradigm='LVD',selfsup=False, segm=0, labels=[], unsup=False, device='cuda'):
        super(WideVoxelOffsetField, self).__init__()
        self._name = 'voxel_encoder'
        self.res = res 
        self.paradigm = paradigm
        self.b_min = torch.FloatTensor(b_min).to(device)
...
class ScaledVoxelOffsetField(FieldNetworkBase):
    def __init__(self, hidden_dim=512, output_dim=6890, res=128, input_dim=1, 
                 b_min = np.array([-1.2, -1.4, -1.2]), b_max = np.array([1.2, 1.3, 1.2]), 
                 paradigm='LVD',selfsup=False, power_factor = 5, device='cuda'):
        super(ScaledVoxelOffsetField, self).__init__()
        self._name = 'voxel_encoder'
        self.res = res 
        self.paradigm = paradigm
        self.b_min = torch.FloatTensor(b_min).to(device)
...
#         self.k = n_basis
#         self.feature_transform = feature_transform
#         self.feat = PointFeatureEncoder(n_in = n_in,n_layers=n_layers,size_layers=size_layers,normalize = False, global_feat=False, feature_transform=feature_transform)
...
class PointConditionedIfNet(nn.Module):
    def __init__(self, in_dim=1, hidden_dim=256, with_displacements=True):
        super(PointConditionedIfNet, self).__init__()
