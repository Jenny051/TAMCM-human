...
from volumetric_bodyfit.config.runtime import faust_registration_dir, faust_scan_dir, challenge_pair_file
import numpy as np
import tqdm
import hydra
...
from volumetric_bodyfit.config.runtime import checkpoint_store, model_profile_store, project_home, registration_output_dir
import os 
...
def run(cfg: DictConfig) -> str:
    ...
    pairs = challenge_pair_file
    file1 = open(pairs, 'r')
    Lines = file1.read().splitlines()
    x = [Lines[i].split('_') for i in range(len(Lines))]

    regs  = faust_registration_dir
    scans = faust_scan_dir
    ...
    for p in tqdm.tqdm(x):
        ...
        c  = threshold_recall_curve(np.hstack(errs).flatten(),tr);
...
####
@hydra.main(config_path=str(PROJECT_ROOT / "profiles"), config_name="default_err")
