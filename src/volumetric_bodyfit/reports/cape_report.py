#鐢ㄦ潵娴嬭瘯缁忚繃partial鎴栬€卻ingleview璁粌杩囩殑fieldfit妯″瀷锛屾湭璁粌鐨勮鐢╛variant
#1.淇敼62琛宲red鏂囦欢鍚?#2.淇敼59琛岀敤鏉ュ仛涓績鍖栫殑scan
#3.淇敼108琛宱s.path.join(pred_folder, name, "outcape_ss_cham_0.ply"), 濡傛灉鏄痯artial娴嬭瘯锛?->1
import os
import glob
...
def main():
    ...
    scan_dir = os.environ.get("BODYFIT_CAPE_ERROR_SCAN_DIR", "datafolder/CAPE/eval_scans")
    ...
    pred_folder = f"output_55f_mixed_partial_d1_1/matchAMASS_CAPE/{exp_name}"
    ...
    for name in tqdm(sorted(os.listdir(pred_folder))):
        ...
        os.rename(
            os.path.join(pred_folder, name, "outcape_ss_cham_1.ply"),
